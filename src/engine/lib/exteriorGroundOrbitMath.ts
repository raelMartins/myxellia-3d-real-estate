...
const _ndcSamples: THREE.Vector2[] = (() => {
    ...
    for (let i = 0; i <= n; i++) {
        const t = -1 + (2 * i) / n;
        pts.push(new THREE.Vector2(t, -1), new THREE.Vector2(t, 1), new THREE.Vector2(-1, t), new THREE.Vector2(1, t));
    }
    pts.push(new THREE.Vector2(0, 0), new THREE.Vector2(-0.65, -0.65), new THREE.Vector2(0.65, -0.65), new THREE.Vector2(0.65, 0.65), new THREE.Vector2(-0.65, 0.65));
    return pts;
})();
...
export function footprintMaxDolly(persp: THREE.PerspectiveCamera, hx: number, hz: number): number {
    const fovV = THREE.MathUtils.degToRad(persp.fov);
    const fovH = 2 * Math.atan(Math.tan(fovV / 2) * persp.aspect);
    const dAxisAligned = Math.min(hx / Math.tan(fovH / 2), hz / Math.tan(fovV / 2));
...
export function footprintMaxDollyWhenPadEditingRect(strictRectangularMax: number): number {
    return THREE.MathUtils.clamp(
        Math.max(strictRectangularMax * PAD_EDIT_ORBIT_MAX_MULT, DEFAULT_EXTERIOR_ORBIT_MAX * 0.92),
        MIN_DIST_FLOOR * 6,
        ABS_MAX_DOLLY
    );
}
...
export function footprintMaxDollyWhenPadEditing(persp: THREE.PerspectiveCamera, hx: number, hz: number): number {
...
function isFrustumGroundInsideRect(
    ...
    for (let i = 0; i < _ndcSamples.length; i++) {
        ...
        const x = o.x + dir.x * t;
        const z = o.z + dir.z * t;
        if (Math.abs(x - cx) > hx + eps || Math.abs(z - cz) > hz + eps) return false;
    }
    return true;
}
...
export function rectangularParcelMaxDolly(
    ...
    const test = (dist: number): boolean => {
        ...
        return isFrustumGroundInsideRect(persp, _camPos, orbitTarget, groundPlaneY, cx, cz, hx, hz, edgeEps);
    };
    ...
    if (!test(lo)) {
        ...
        return THREE.MathUtils.clamp(seed * FOOTPRINT_SLACK, loFloor, ABS_MAX_DOLLY);
    }
...
export function rectangularParcelMaxDollyAlongUnitRay(
    ...
    const probe = _camPos.copy(orbitTarget).addScaledVector(unitFromTargetToCamera, Math.max(MIN_DIST_FLOOR * 8, 1));
    return rectangularParcelMaxDolly(persp, probe, orbitTarget, cx, cz, hx, hz, groundPlaneY, edgeEps);
}

/** Union AABB of meshes tagged `userData.isWorldGround` (imported ground only; see file header). */
export function expandWorldGroundBox(scene: THREE.Object3D, box: THREE.Box3): void {
    box.makeEmpty();
 ...
 */
export function expandWorldRootGroundFootprintFallback(root: THREE.Object3D, box: THREE.Box3): void {
    box.makeEmpty();
...
export function snapDefaultObliqueExterior(
    ...
    const maxAlongRay = rectangularParcelMaxDollyAlongUnitRay(camera, center, offsetDir, hx, hz, planeY, center.x, center.z);
