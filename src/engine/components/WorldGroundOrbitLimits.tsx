...
export default function WorldGroundOrbitLimits({
  ...
  const placementPadEditActive = useEngineStore(
    (s) => s.placementPadEditActive,
  );
  ...
  useFrame(() => {
    ...
    oc.maxDistance = placementPadEditActive
      ? footprintMaxDollyWhenPadEditingRect(strictRectMax)
      : strictRectMax;

    const minByExtent = Math.max(
      Math.min(hx, hz) * MIN_DIST_HALF_EXTENT_MULT,
      MIN_DIST_FLOOR,
    );
